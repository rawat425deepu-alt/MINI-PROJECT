/* ============================================================
 * SportsOPS - Standalone C Backend (Console Edition)
 * ------------------------------------------------------------
 * Menu-driven Sports Management System with file-based storage.
 * Uses fixed-width binary records for fast CRUD operations.
 *
 * Build:   gcc -std=c11 -O2 -o sports sports.c
 * Run:     ./sports          (data files created in ./data/)
 *
 * Features:
...
typedef struct {
    ...
    int  scoreA;
    int  scoreB;
    int  status;            /* 0=scheduled 1=live 2=completed */
    int  tournamentId;      /* 0 = none */
} Match;
...
typedef struct {
    ...
    char startDate[12];
    char endDate[12];
    int  status;            /* 0=upcoming 1=ongoing 2=completed */
} Tournament;
